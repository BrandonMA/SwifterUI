...
open class SFContactsViewController: SFViewController {
    ...
    open lazy var contactsManager: SFTableManager<SFUser, SFTableViewContactCell, SFTableViewHeaderView, SFTableViewFooterView> = {
        ...
        manager.configure(tableView: tableView, cellStyler: { (cell, user, indexPath) in
            ...
            } else if let string = user.profilePictureURL, let url = URL(string: string) {
...
extension SFContactsViewController: UISearchResultsUpdating {
    ...
    public func updateSearchResults(for searchController: UISearchController) {
        ...
        if text == "" {
            contactsManager.forceUpdate(dataSections: user.contacts.ordered())
        } else {
            contactsManager.forceUpdate(dataSections: user.contacts.filter(by: text).ordered())
        }
    }
...
extension SFContactsViewController: SFTableManagerDelegate {
    ...
    public func didSelectRow<DataType>(at indexPath: IndexPath, tableView: SFTableView, item: DataType) where DataType: Hashable {
        ...
        if let existingChat = user.chats.first(where: { $0.users.contains(item.identifier) && $0.users.count == 2 }) {
            chat = existingChat
        } else {
            chat = SFSingleChat(currentUser: user, contact: item)
        }
        
        if let navigationController = navigationController {
            navigationController.viewControllers.forEach { (controller) in
                if controller.isKind(of: SFConversationsTableViewController.self) || controller.isKind(of: SFConversationsCollectionViewController.self) {
